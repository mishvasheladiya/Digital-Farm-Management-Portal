...
/* ---------- 3. FETCH DATA & ANALYTICS (Updated logic) ---------- */
/** * Logic: confirmed_order will only be non-null if an order exists 
 * that is NOT 'Cancelled'. If a distributor rejects an order, 
 * the product becomes "available" for confirmation again.
 */
$query = "SELECT p.*, 
          ...
          FROM products p 
          WHERE p.farmer_id = ? AND p.category = 'Vegetable' 
...
<head>
    ...
    <title>Vegetable Inventory | GreenAgro Pro</title>
...
<main class="max-w-7xl mx-auto px-6 py-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div class="bg-white p-6 rounded-[2rem] border border-emerald-100 shadow-sm flex items-center gap-5">
            <div class="bg-emerald-50 p-4 rounded-2xl text-emerald-600"><i data-lucide="leaf" class="w-6 h-6"></i></div>
            <div>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Varieties</p>
                <p class="text-2xl font-extrabold text-slate-800"><?= $total_items ?> Crops</p>
            </div>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-emerald-100 shadow-sm flex items-center gap-5">
            <div class="bg-emerald-50 p-4 rounded-2xl text-emerald-500"><i data-lucide="package" class="w-6 h-6"></i></div>
            <div>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Total Volume</p>
                <p class="text-2xl font-extrabold text-slate-800"><?= number_format($total_stock) ?> <span class="text-sm font-medium">kg</span></p>
            </div>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-emerald-100 shadow-sm flex items-center gap-5">
            <div class="bg-emerald-50 p-4 rounded-2xl text-emerald-600"><i data-lucide="trending-up" class="w-6 h-6"></i></div>
            <div>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Asset Value</p>
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
            <h1 class="text-4xl font-extrabold tracking-tight text-slate-900 uppercase">vegetable<span class="text-emerald-500 font-light">inventory</span></h1>
            <p class="text-slate-500 mt-2 font-medium">Manage your fresh produce stock levels and market pricing.</p>
        </div>
        <a href="Catalog.php" class="flex items-center gap-2 bg-slate-900 text-white px-8 py-4 rounded-2xl font-bold hover:bg-emerald-600 shadow-lg transition-all active:scale-95">
    ...
    <div class="space-y-4">
        <div class="hidden md:grid grid-cols-12 px-10 py-3 text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">
            <div class="col-span-5">Vegetable Details</div>
            <div class="col-span-2">Stock Quantity</div>
        ...
        <?php if ($total_items > 0): ?>
            <?php foreach ($crops as $item): 
                $img_path = !empty($item['image_path']) ? "../" . $item['image_path'] : "https://placehold.co/400x400?text=Veg";
                $low_stock = ($item['stock_quantity'] < 25);
            ?>
            <div class="item-row bg-white/60 border border-slate-100 rounded-[2.5rem] p-5 md:px-10 md:py-6 grid grid-cols-1 md:grid-cols-12 items-center gap-6">
                ...
                <div class="col-span-1 md:col-span-5 flex items-center gap-6">
                    <img src="<?= $img_path ?>" 
                         onerror="this.src='https://placehold.co/400x400/ecfdf5/059669?text=Veg'"
                         class="w-20 h-20 rounded-[1.8rem] object-cover shadow-sm border-2 border-white" alt="Veg">
                    <div>
                        ...
                        <span class="inline-flex items-center gap-1.5 text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <i data-lucide="map-pin" class="w-3.5 h-3.5 text-emerald-500"></i> <?= htmlspecialchars($item['farm_location']) ?>
                        </span>
                    </div>
                </div>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <div class="text-2xl font-black text-slate-900">
                        <span class="text-emerald-500 text-sm font-bold mr-0.5">₹</span><?= number_format($item['price'], 2) ?>
                    </div>
                </div>
                ...
                <div class="col-span-1 md:col-span-2 flex flex-col items-center">
                    ...
                    <?php if($item['confirmed_order']): ?>
                         <div class="flex flex-col items-center">
                            ...
                            <span class="text-emerald-600 text-[9px] font-black uppercase"><?= $item['current_order_status'] ?></span>
                         </div>
                    <?php elseif($item['interest_count'] > 0): ?>
                        <button onclick="openInterestModal(<?= $item['id'] ?>, '<?= htmlspecialchars($item['product_name']) ?>')" 
                                class="bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-900 transition-all shadow-md flex items-center gap-2">
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <div class="py-32 text-center bg-white rounded-[4rem] border border-emerald-100 shadow-sm">
                <div class="bg-emerald-50 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6">
                    <i data-lucide="sprout" class="w-10 h-10 text-emerald-200"></i>
                </div>
                <h3 class="text-2xl font-bold text-slate-800 tracking-tight">Vault is Empty</h3>
                <a href="../Catalog.php" class="inline-block mt-8 bg-emerald-600 text-white px-10 py-4 rounded-full font-bold shadow-lg hover:bg-emerald-700 transition-all">List First Crop</a>
            </div>
        <?php endif; ?>
    </div>
</main>
...
<div id="interestModal" class="fixed inset-0 z-[100] hidden flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
    <div class="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        <div class="px-8 py-6 bg-slate-900 text-white flex justify-between items-center">
            <div>
                <h2 class="text-xl font-bold" id="modalProdName">Interests</h2>
                <p class="text-emerald-400 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Verified Distributors</p>
            </div>
            <button onclick="closeModal()" class="p-2 hover:bg-slate-800 rounded-xl transition-colors">
...
<script>
    ...
    function openInterestModal(productId, productName) {
        ...
        list.innerHTML = `<div class="py-10 text-center"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-500 mx-auto"></div></div>`;
            ...
            .then(data => {
                if(data.length > 0) {
                    list.innerHTML = data.map(d => `
                        <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-emerald-200 transition-colors">
                            <div>
                                <p class="font-bold text-slate-800">${d.first_name} ${d.last_name}</p>
                                <p class="text-[10px] font-bold text-emerald-600 uppercase tracking-tight">${d.company_name} • ${d.city}</p>
                            </div>
                            <button onclick="confirmDeal(${d.distributor_id}, ${productId})" 
                                    class="bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-900 shadow-sm transition-all">
                                Confirm
                            </button>
                        </div>
                    `).join('');
    ...
    function confirmDeal(distId, prodId) {
        if(confirm("Confirm this distributor? This will initiate the order process.")) {
