...
/* ---------- 3. FETCH DATA (Updated Logic to handle Cancelled Orders) ---------- */
/**
 * Logic: confirmed_order only picks up orders that are 'Confirmed' or 'Completed'.
 * If an order is 'Cancelled', this subquery returns null, 
 * making the "Interested" button reappear.
 */
$query = "SELECT p.*, 
          ...
          FROM products p 
          WHERE p.farmer_id = ? AND p.category = 'Spice' 
...
<head>
    ...
    <title>Spice Inventory | GreenAgro Pro</title>
    ...
    <style>
        body { background-color: #fffaf9; color: #1e293b; font-family: 'Plus Jakarta Sans', sans-serif; }
        .item-row { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); border: 1px solid transparent; }
        .item-row:hover {
            background-color: #ffffff;
            box-shadow: 0 20px 25px -5px rgba(220, 38, 38, 0.06);
...
<main class="max-w-7xl mx-auto px-6 py-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div class="bg-white p-6 rounded-[2rem] border border-red-100 shadow-sm flex items-center gap-5">
            <div class="bg-red-50 p-4 rounded-2xl text-red-600"><i data-lucide="sparkles" class="w-6 h-6"></i></div>
            <div>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Spice Varieties</p>
                <p class="text-2xl font-extrabold text-slate-800"><?= $total_items ?> Items</p>
            </div>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-red-100 shadow-sm flex items-center gap-5">
            <div class="bg-red-50 p-4 rounded-2xl text-red-500"><i data-lucide="scale" class="w-6 h-6"></i></div>
            <div>
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-red-100 shadow-sm flex items-center gap-5">
            <div class="bg-red-50 p-4 rounded-2xl text-red-600"><i data-lucide="gem" class="w-6 h-6"></i></div>
            <div>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Net Asset Value</p>
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
            <h1 class="text-4xl font-extrabold tracking-tight text-slate-900 uppercase">Spice<span class="text-red-500 font-light ml-2">Vault</span></h1>
            <p class="text-slate-500 mt-2 font-medium">Manage your premium spice inventory, batches, and aromatic harvest dates.</p>
        </div>
        <a href="Catalog.php" class="flex items-center gap-2 bg-slate-900 text-white px-8 py-4 rounded-2xl font-bold hover:bg-red-600 shadow-lg transition-all active:scale-95">
    ...
    <div class="space-y-4">
        <div class="hidden md:grid grid-cols-12 px-10 py-3 text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">
            <div class="col-span-5">Aromatic Profile</div>
            <div class="col-span-2">Stock Level</div>
            <div class="col-span-2">Premium Price</div>
        ...
        <?php if ($total_items > 0): ?>
            <?php foreach ($crops as $item): 
                $img_path = !empty($item['image_path']) ? "../" . $item['image_path'] : "https://placehold.co/400x400?text=Spice";
                $low_stock = ($item['stock_quantity'] < 5); 
            ?>
            <div class="item-row bg-white/60 border border-slate-100 rounded-[2.5rem] p-5 md:px-10 md:py-6 grid grid-cols-1 md:grid-cols-12 items-center gap-6">
                ...
                <div class="col-span-1 md:col-span-5 flex items-center gap-6">
                    <img src="<?= $img_path ?>" 
                         onerror="this.src='https://placehold.co/400x400/fef2f2/dc2626?text=Spice'"
                         class="w-20 h-20 rounded-[1.8rem] object-cover shadow-sm border-2 border-white" alt="Spice">
                    <div>
                        ...
                        <span class="inline-flex items-center gap-1.5 text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <i data-lucide="flame" class="w-3.5 h-3.5 text-red-500"></i> Region: <?= htmlspecialchars($item['farm_location']) ?>
                        </span>
                    </div>
                </div>

                <div class="col-span-1 md:col-span-2">
                    <p class="text-[10px] font-bold text-slate-400 uppercase mb-2 md:hidden">Inventory</p>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <div class="text-2xl font-black text-slate-900">
                        <span class="text-red-500 text-sm font-bold mr-0.5">₹</span><?= number_format($item['price'], 2) ?>
                    </div>
                </div>

                <div class="col-span-1 md:col-span-2 flex flex-col items-center">
                    <p class="text-[10px] font-bold text-slate-400 uppercase mb-2 md:hidden">Interest</p>
                    <?php if($item['confirmed_order']): ?>
                        <div class="flex flex-col items-center">
                            ...
                            <span class="text-red-600 text-[9px] font-black uppercase"><?= $item['current_order_status'] ?></span>
                        </div>
                    <?php elseif($item['interest_count'] > 0): ?>
                        <button onclick="openInterestModal(<?= $item['id'] ?>, '<?= htmlspecialchars($item['product_name']) ?>')" 
                                class="bg-slate-900 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-red-600 transition-all shadow-md flex items-center gap-2">
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <div class="py-32 text-center bg-white rounded-[4rem] border border-red-100 shadow-sm">
                <div class="bg-red-50 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6">
                    <i data-lucide="component" class="w-10 h-10 text-red-200"></i>
                </div>
                <h3 class="text-2xl font-bold text-slate-800 tracking-tight">Spice Vault Empty</h3>
                <a href="Catalog.php" class="inline-block mt-8 bg-red-600 text-white px-10 py-4 rounded-full font-bold shadow-lg hover:bg-red-700 transition-all">Start Listing</a>
            </div>
        <?php endif; ?>
    </div>
</main>
...
<div id="interestModal" class="fixed inset-0 z-[100] hidden flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
    <div class="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        <div class="px-8 py-6 bg-red-600 text-white flex justify-between items-center">
            <div>
                <h2 class="text-xl font-bold" id="modalProdName">Spice Lead Details</h2>
                <p class="text-red-100 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Market Connections</p>
            </div>
            <button onclick="closeModal()" class="p-2 hover:bg-red-500 rounded-xl transition-colors">
...
<script>
    ...
    function openInterestModal(productId, productName) {
        ...
        list.innerHTML = `<div class="py-10 text-center"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-red-500 mx-auto"></div></div>`;
            ...
            .then(data => {
                if(data.length > 0) {
                    list.innerHTML = data.map(d => `
                        <div class="flex items-center justify-between p-4 bg-orange-50/30 rounded-2xl border border-orange-100 hover:border-red-200 transition-colors">
                            <div>
                                <p class="font-bold text-slate-800">${d.first_name} ${d.last_name}</p>
                                <p class="text-[10px] font-bold text-red-600 uppercase tracking-tight">${d.company_name} • ${d.city}</p>
                            </div>
                            <button onclick="confirmDeal(${d.distributor_id}, ${productId})" 
                                    class="bg-slate-900 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-red-600 shadow-sm transition-all">
                                Confirm
                            </button>
                        </div>
                    `).join('');
    ...
    function confirmDeal(distId, prodId) {
        if(confirm("Process this spice deal? The distributor will be notified.")) {
